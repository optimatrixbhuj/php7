<?php
	...
	class model extends Singleton{
		...
		function map_fields($mapping=array(), $only_map_data = false){
			if(!is_array($mapping)){
				$this->app->display_error(debug_backtrace(), "The argument \$mapping should be an array");
			}
			$this->fieldmapping = $mapping;
			/* UPDATED DONE FOR CONSIDER ON MAPING DATA AND DONT CHECK GET/POST VALUES IN INSERT/UPDATE QUERY.
			BY REEMA MEHTA : 26-6-2020 */
			if($only_map_data==true){
				$this->only_custom_data=true;
			}
		}

		

		/* UPDATE PATCH ADDED mysqli_real_escape_string FOR SELECT QUERY FOR HIDE MYSQL INJECTION IN LOGIN 

			ADDED BY VISHAL SHAH ON 21/4/2014
		...
		*/
		...
		function record_exists($field_value=NULL, $return_field=NULL){
			...
			if(!is_array($field_value) && $this->model_data->ID==0){
				...
				$strWhereClause.="AND (".mysqli_real_escape_string($this->app->objDB->getConnection(),$field_value).")";
							...
							switch($datatype){
								...
								case "decimal":
									...
									if($value!=""){
										...
										$strWhereClause.=" AND `".$key."`=".mysqli_real_escape_string($this->app->objDB->getConnection(),$value);
							...
							if($this->model_data->KEY!=$key){
								...
								if(array_key_exists($key, $this->fieldmapping)){
									...
									if($this->app->getPostVar($key)!==NULL){
										...
										if($key=='admin_user_id' && isset($_SESSION['admin_user_id']) && VIR_DIR!=""){

										//continue;

											$value=$_SESSION['admin_user_id'];
									...
									case "longblob":
										...
										if($value!==NULL){
											...
											if(!get_magic_quotes_gpc()){
												...
												$value = mysqli_real_escape_string($this->app->objDB->getConnection(),stripslashes($value));
		...
		/* GET TOTAL RECORDS :: SIMILAR TO mysqli_num_rows

			ADDED BY REEMA MEHTA ON 06/03/2019	*/

		function getTotalRecords($return=true){

			if($return){

				return $this->app->objDB->getTotalRecords("records_".get_class($this), true);

			}else{

				$this->app->objDB->getTotalRecords("records_".get_class($this), false);
