...
class dbclass extends Singleton{
	...
	public function open($is_new_connection=false){
		if($this->db_host!="" && $this->db_database!="" && $this->db_username!="" && $this->db_password!=""){
			...
			//mysqli_select_db($this->db_database, $this->CONNECTION) or die($this->error());
		}else{
	...
	public function close(){
		...
		$_SESSION['error_string']=$file_content;
		}*/
		if($this->CONNECTION){
	...
	private function error(){
		...
		$str.= "<strong>Error Description :</strong><hr>Error No : ".(!$this->CONNECTION?mysqli_connect_errno():mysqli_errno($this->CONNECTION))."<br>Description : ".(!$this->CONNECTION?mysqli_connect_error():mysqli_error($this->CONNECTION))."<br><br>";
		if($this->SQL!=""){
	...
	public function execute($recordset_name="", $auto_paging=false, $field_to_count="", $record_per_page=0, $segment_length=0){
		...
		if((preg_match("/^SELECT/", $this->SQL) || preg_match("/^\(SELECT/", $this->SQL)) && $auto_paging===true){
			if($field_to_count==""){
				...
				$starttime = microtime(); //
				$result = mysqli_query($this->CONNECTION, $this->SQL) or die($this->error());
				////// CODE TO WRITE SQL STATEMENTS IN HTML FILE TO CHECK THE EXECUTION TIME OF QUERY /////
				if(SQL_STATEMENTS_LOG == "yes"){
					$endtime = microtime(); //
					$time_taken = $endtime - $starttime;	// Consider $starttime from above query line
					$handle = fopen(ABS_PATH."/logs/sql_statements.txt", 'a'); //
					$color_set = "#00FF00";
					if($time_taken > 0.005)
						$color_set = "#FF0000";
					fwrite($handle, "<span style='color:".$color_set."'>".str_pad($time_taken,10,0)." Seconds >> [".date("Y-m-d")."] >> ".$this->SQL."</span><br>");
				}
				////// CODE TO WRITE SQL STATEMENTS IN HTML FILE TO CHECK THE EXECUTION TIME OF QUERY /////
				$total_records = 0;
				...
				if($total_records>0){
					...
					if(count($this->groups)>0){
						...
						array_push($this->QUERY_STACK, $this->SQL);
						$starttime = microtime(); //
						$rs_tmp = mysqli_query($this->CONNECTION, $this->SQL) or die($this->error());
						////// CODE TO WRITE SQL STATEMENTS IN HTML FILE TO CHECK THE EXECUTION TIME OF QUERY /////
						if(SQL_STATEMENTS_LOG == "yes"){
							$endtime = microtime(); //
							$time_taken = $endtime - $starttime;	// Consider $starttime from above query line
			...
			if($ToCounter > $FromCounter){
				...
				if($this->RESULTS[$recordset_name]['current_segment']>=2){
					if($this->custom_paging_function == NULL){
						$First_Link = "<a href='".$Current_Page."?pg_no=0".$Query_String."'>First</a>&nbsp;";
						$Prev_Link = "<a href='".$Current_Page."?pg_no=".($FromCounter-1).$Query_String."'>Prev</a>";
					}else{
						$First_Link = "<a href='".$Current_Page."?pg_no=0".$Query_String."'>First</a>&nbsp;";
						$Prev_Link = "<a href='javascript:void(0)' onclick='".$this->custom_paging_function."(".($FromCounter-1).")' >Prev</a>";
					}
				}else{
					$First_Link .= "<a class='no_link'>First</a>&nbsp;";
					$Prev_Link = "<a class='no_link'>Prev</a>";
				}
				...
				for($i=$FromCounter; $i<$ToCounter; $i++){
					...
					if($i==$this->CURRENT_PAGE){
						...
						if($this->custom_paging_function == NULL){
							$Page_Link.="<a href='".$Current_Page."?pg_no=".$i.$Query_String."' >".($i+1)."</a>";
						}else{
							$Page_Link.="<a href='javascript:void(0)' onclick='".$this->custom_paging_function."(".$i.")' >".($i+1)."</a>";
						}
					}
				}
				...
				if($this->RESULTS[$recordset_name]['current_segment']<$Segments){
					if($this->custom_paging_function == NULL){
						$Next_Link.= "<a href='".$Current_Page."?pg_no=".$ToCounter.$Query_String."' >Next</a>";
						$My_Last_Page = $TotalPages-1;
						$Last_Link.= "<a href='".$Current_Page."?pg_no=".$My_Last_Page.$Query_String."'>Last</a>";
					}else{
						$Next_Link.= "<a href='javascript:void(0)' onclick='".$this->custom_paging_function."(".$ToCounter.")' >Next</a>";
						$My_Last_Page = $TotalPages-1;
						$Last_Link.= "<a href='".$Current_Page."?pg_no=".$My_Last_Page.$Query_String."'>Last</a>";
					}
				}else{
					$Next_Link.= "<a class='no_link'>Next</a>";
					$Last_Link.= "<a class='no_link'>Last</a>";
				}
				
				if($return){
					return $First_Link.$Prev_Link.$Page_Link.$Next_Link.$Last_Link;
				}else{
					echo $First_Link.$Prev_Link.$Page_Link.$Next_Link.$Last_Link;
				}
			}
		}
	}
	...
	/* GET CONNECTION OBJECT VARIABLE TO USE IN mysqli_query FUNCTION FOR ANY DIRECT QUERY
			ADDED BY REEMA MEHTA ON 24/12/2019		*/
	function getConnection($return=true){
		if($return){
