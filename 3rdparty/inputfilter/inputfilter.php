...

{
...

	{
		...
		//$source = preg_replace('/&#(\d+);/me', "chr(\\1)", $source); // decimal notation // old version

		// convert hex

		//$source = preg_replace('/&#x([a-f0-9]+);/mei', "chr(0x\\1)", $source); // hex notation  // old version

		// convert decimal

		$source = preg_replace_callback('/&#(\d+);/m', function ($m) { return chr(hexdec($m[1])); }, $source); // decimal notation
