...
class imageresizer {
	...
	function loadImage($imgPath) {
		...
		/*======= code update by Reema on 31-12-2020 
		 check for image data. If image file have other data like php code then go to else block and remove file from temp folder
		 ========= */
		if(is_array($this->_imgData) && count($this->_imgData)>0){
			...
			if(file_exists($this->_baseDir.$imgPath)){
				@unlink($this->_baseDir.$imgPath);
			}
			return false;
		}
	}
