 ...
 *--------------------------------------------------------------------
 * $Id: BCGothercode.barcode.php,v 1.7 2008/03/27 02:39:10 jsgoupil Exp $
 * PHP5-Revision: 1.8
 *--------------------------------------------------------------------
	...
	function drawText(&$im) {
		if($this->label !== $this->AUTO_LABEL && $this->label !== '') {
			if(is_a($this->textfont, 'BCGFont')) {
				...
				$xPosition = (imagesx($im) - $this->offsetX * $this->scale) / 2 - $textfont->getWidth() / 2 + $this->offsetX * $this->scale;
				...
				$xPosition = (imagesx($im) - $this->offsetX * $this->scale) / 2 - (strlen($this->label) * imagefontwidth($this->textfont)) / 2 + $this->offsetX * $this->scale;

				$text_color = $this->colorFg->allocate($im);
				imagestring($im, $this->textfont, $xPosition, $this->thickness * $this->scale, $this->label, $text_color);
			}
		}
	}
};
