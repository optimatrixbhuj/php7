 ...
 *--------------------------------------------------------------------
 * $Id: BCGpostnet.barcode.php,v 1.8 2008/03/27 02:39:10 jsgoupil Exp $
 * PHP5-Revision: 1.10
 *--------------------------------------------------------------------
	...
	function draw(&$im) {
		...
		if($error_stop === false) {
			...
			if($error_stop === false) {
				...
				// Starting Code
				$this->drawChar($im, '1', false);
				// Code
				for($i = 0; $i < $c; $i++) {
					$this->drawChar($im, $this->findCode($this->text[$i]), false);
				}
				// Checksum
				$this->drawChar($im, $this->findCode($checksum), false);
				//Ending Code
