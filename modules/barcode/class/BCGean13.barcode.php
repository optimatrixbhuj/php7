 ...
 *--------------------------------------------------------------------
 * $Id: BCGean13.barcode.php,v 1.11 2008/03/27 02:39:10 jsgoupil Exp $
 * PHP5-Revision: 1.11
 *--------------------------------------------------------------------
...
class BCGean13 extends BCGBarcode1D {
	...
	function drawBars(&$im) {
		...
		for($i = 0; $i < 5; $i++) {
			$this->drawChar($im, BCGean13::inverse($this->findCode($temp_text[$i + 2]), $this->codeParity[$temp_text[0]][$i]), false);
		}
	...
	function drawExtendedBars(&$im, $plus, &$code1, &$code2) {
		...
		$this->thickness = $this->thickness + $plus;
		$this->positionX = 0;
