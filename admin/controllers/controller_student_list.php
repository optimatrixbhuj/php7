...
class _student_list extends controller{		
	...
	function load_data(){
		...
		$obj_model_student->set_paging_settings($_SESSION['records'],5);	
		...
		if($this->app->getPostVar("name")!=""){
			...
			$_SESSION['student_search']['name'] = $this->app->getPostVar("name");

		}		

		$sql_where_clause = "";

		if(isset($_SESSION['student_search']['name'])){

			$name = $_SESSION['student_search']['name'];

			$sql_where_clause .=($sql_where_clause!=''?" AND ":""). "name like '%".$name."%'";

			$this->assign("field_name", $_SESSION['student_search']['name']);
...

		{
			...
			$_SESSION['order_by_field_name'] = "student.id";	
		...
		if($this->app->getGetVar('recycle')== true){
			...
			$sql_where_clause=($sql_where_clause==''?"student.status='Inactive'":"student.status='Inactive' and ".$sql_where_clause);

		}else{

			$sql_where_clause=($sql_where_clause==''?"student.status='Active'":"student.status='Active' and ".$sql_where_clause);

		}

	

		$rs = $obj_model_student->execute("SELECT", true, "", $sql_where_clause, $sql_order_by_clause);		

		$data = $this->app->compile();

		$this->load_parser($data);

		$this->parser->assign("PAGING", $obj_model_student->show_paging());
		...
		$Sr = $obj_model_student->get_serial_start();
		...
		foreach($rs as $student){
			...
			$this->parser->assign("STATUS", ($student['status'])=="Active"?"active.png":"inactive.png");

			$this->parser->assign("COUNT", $i);

			$this->parser->assign("SERIAL", $Sr);

			$this->parser->parse('main.student_table.student_row');
		...
		$this->parser->parse('main.student_table');		
	...
	function multi_delete(){
		...
		for($i=0;$i<count($del_id);$i++){
			$is_exist=$this->app->utility->check_record_used_delete("student",$del_id[$i],"student");
			if ($is_exist=='Yes') {
				$del_status = 'False';
			}else{
				...
				$is_delete[] = $obj_model_student->execute("UPDATE", false, "", "id=".$del_id[$i]);

				//add user log for DELETE

				$this->app->utility->add_user_log("student",$del_id[$i],"DELETE");
			}
	...
	function single_delete(){
		$is_exist=$this->app->utility->check_record_used_delete("student",$this->app->getPostVar('id'),"student");
		if ($is_exist=='Yes') {
			...
			if($obj_model_student->execute("UPDATE")){

				//add user log for DELETE

				$this->app->utility->add_user_log("student",$this->app->getPostVar('id'),"DELETE");
		...
		$this->app->redirect("index.php?pg_no=".$this->app->getPostVar('page_no')."&view=student_list");			
	...
	function recycle(){
		$is_exist=$this->app->utility->check_record_exist_recycle("student",$this->app->getPostVar('id'),'name');
		if ($is_exist=='Yes') {
			...
			if($obj_model_student->execute("UPDATE")){
				...
				$this->app->utility->add_user_log("student",$this->app->getPostVar('id'),"RESTORE");
