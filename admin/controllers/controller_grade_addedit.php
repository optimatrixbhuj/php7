<?php
class _grade_addedit extends controller{
	...
	function load_data(){
		$obj_model_grade = $this->app->load_model("grade", $this->app->getGetVar('id'));
		$rs_grade = $obj_model_grade->execute("SELECT",false,"","","","");
		if(count($rs_grade)>0){
			$this->app->assign_form_data("frm_grade_addedit", $rs_grade[0]);
		}else{
	...
	function addedit_data(){			
		if($this->app->getPostVar('id')!=""){
			...
			if($obj_model_grade->execute("UPDATE")>0){
				//add user log for UPDATE
				$this->app->utility->add_user_log("grade",$this->app->getPostVar('id'),"UPDATE",$this->app->getPostVars());
				$this->app->utility->set_message("Record updated successfully", "SUCCESS");
				if($this->app->getPostVar('apply_x') != ""){
					$this->app->redirect("index.php?view=grade_addedit&id=".$this->app->getPostVar('id'));
				}else{
					$this->app->redirect("index.php?view=grade_list&pg_no=".$this->app->getGetVar('page_no'));
				}
			}else{
			...
			$obj_model_grade->map_fields($add_field);
			$inserted_id = $obj_model_grade->execute("INSERT");
			if($inserted_id>0){
				//add user log for INSERT
				$this->app->utility->add_user_log("grade",$inserted_id,"INSERT",$this->app->getPostVars());
				$this->app->utility->set_message("Record inserted successfully", "SUCCESS");
				if($this->app->getPostVar('apply_x') != ""){
					$this->app->redirect("index.php?view=grade_addedit&id=".$inserted_id);
				}else{
					$this->app->redirect("index.php?view=grade_list");
				}
			}else{
